      ...
      link.each do |sink_source|
        case sink_source.name
          when "sink"
            ...
            if sink_source["type"] == "processor"
              datalink.sink_port = sink_source.last.content
              datalink.sink += ":" + datalink.sink_port
            else
              datalink.sink_port = nil
            end
          when "source"
            datalink.source = sink_source.first.content
            if sink_source["type"] == "processor"
              datalink.source_port = sink_source.last.content
              datalink.source += ":" + datalink.source_port
            else
              datalink.source_port = nil
            end
        end
      end
