module T2Flow
  ...
  class Dot
    ...
    def initialize(port_style = 'none')
      # @port_style IS CURRENTLY UNUSED. IGNORE!!!
      @port_style = port_style # 'all', 'bound' or 'none'
    end
	...
	if @port_style == 'none'
	  ...
	  label += '}}'
        ...
        stream.puts "  shape=\"#{shape}\","
        stream.puts '  style="filled",'
        stream.puts '  height="0",'
        stream.puts '  width="0",'
        stream.puts "  label=\"#{label}\""
        stream.puts ' ];'
      end
    end
    ...
    def write_source(stream, source, prefix) # :nodoc:
      ...
      stream.puts "   shape=\"#{ (@port_style == 'none') ? 'box' : 'invhouse'}\","
      stream.puts "   label=\"#{source.name}\""
      stream.puts '   width="0",'
      stream.puts '   height="0",'
      stream.puts '   fillcolor="#8ed6f0"'
      stream.puts ' ]' 
    end
    ...
    def write_sink(stream, sink, prefix) # :nodoc:
      ...
      stream.puts "   shape=\"#{ (@port_style == 'none') ? 'box' : 'house'}\","
    ...
    def write_link(stream, link, dataflow, prefix) # :nodoc:
      if dataflow.sources.select{|s| s.name == link.source} != []
        ...
        if "#{processor.type}" == "workflow"
          ...
          if @port_style == 'none' or link.source_port.nil?
            stream.write " \"#{prefix}#{processor.name}\""
          else
            stream.write " \"#{prefix}#{processor.name}\":\"o#{link.source_port}\""
          end
        end
      end
      ...
      if dataflow.sinks.select{|s| s.name == link.sink} != []
        ...
        if "#{processor.type}" == "workflow"
          ...
          if @port_style == 'none' or link.sink_port.nil?
            stream.write "\"#{prefix}#{processor.name}\""
          else
            stream.write "\"#{prefix}#{processor.name}\":\"i#{link.sink_port}\""
          end
        end
      end
      stream.puts ' ['
